...
for(eachsp in 1:length(uniquespp)){
	lmhere<-lm(fitness_z~phenodiff, data=subset(yano, intid==uniquespp[eachsp]))
...
for (n in 1:N){
     s <- intxn[n]
    ypred[n] <- a[intxn[s]] + b[intxn[s]]*year[n] # one way to handle the non-pooled intercepts, there may be other ways
}
